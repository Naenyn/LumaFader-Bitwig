...
from settings import (
    ...
    FOCUS_LAYER_HOLD_PRIORITY,
    FOUR_TRACK_FADER_LAYER_IDS,
    FOUR_TRACK_LAYER_HOLD_PRIORITY,
    settings,
)
...
class GestureEngine:
    ...
    def __init__(self, buttons):
        ...
        self.active_four_track_layer = "sends"
    ...
    def _detect_overlay_holds(self, mode_id):
        ...
        return self._detect_four_track_fader_layers()
    ...
    def _detect_four_track_fader_layers(self):
        """Map held buttons → Four-Track fader layer; emit matching overlay ACTION_CCs."""
        layer_map = settings.get_four_track_fader_layers()
        fine_btn = settings.get_fine_modifier_button()
        held_layers = []
        for layer_id in FOUR_TRACK_FADER_LAYER_IDS:
            trigger = layer_map.get(layer_id, "default")
            if trigger == "default":
                continue
            btn_idx = int(trigger)
        ...
        active = set()
        for layer_id in held_layers:
            action = settings.four_track_layer_overlay_action(layer_id)
            if action:
                active.add(action)

        self.active_four_track_layer = "sends"
        for layer_id in FOUR_TRACK_LAYER_HOLD_PRIORITY:
            if layer_id in held_layers:
                self.active_four_track_layer = layer_id
                break
        return active
